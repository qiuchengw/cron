...
namespace helper {
	...
	inline uint32_t howManySecs(int nX, WCHAR cUnit) {
		switch (cUnit) {
		case L's':
		case L'S':
			return nX;
	...
	inline mstring timeUnitStr(WCHAR cUnit) {
		switch (cUnit) {
		case L's':
		case L'S':
			return "秒";
	...
	*          cUnit   单位部分
	**/
	inline bool parseUnitTime(__in const CStdStringW& src, __out int &nT, __out wchar_t& cUnit) {
		CStdStringW tmp = src;
		if (tmp.Trim().IsEmpty())
			return false;
